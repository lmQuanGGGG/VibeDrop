    ...
    <div className="flex flex-col gap-32 pb-32 max-w-5xl mx-auto relative">
      {prompts.map((prompt, idx) => {
        ...
        return (
          <div 
            key={prompt.id} 
            className="sticky top-4 md:top-24 transition-all duration-300 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] border-4 border-black w-full"
            style={{ zIndex: idx }}
          >
          <div 
            className={`block w-full min-h-[280px] sm:min-h-[400px] md:min-h-[450px] ${color} overflow-hidden group cursor-pointer`}
              ...
              <div className="flex flex-col md:flex-row h-full">
                ...
                <div className="w-full md:w-2/5 border-b-4 md:border-b-0 md:border-r-4 border-black relative bg-black shrink-0 z-10 flex items-center justify-center min-h-[250px] md:min-h-full">
                  {hasImage ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center p-6 text-center bg-neutral-100">
                      <span className="font-display font-black text-2xl sm:text-4xl uppercase break-words line-clamp-3">
                        TXT
                      </span>
                    </div>
                  )}
                ...
                {/* Content Area */}
                <div className="w-full md:w-3/5 p-3 sm:p-6 md:p-8 flex flex-col justify-between relative z-10 flex-grow">
                  <div>
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 mb-1.5 sm:mb-4">
                      <span className="font-mono text-[9px] sm:text-xs font-black uppercase bg-black text-white px-2 py-0.5 sm:px-2.5 sm:py-1 inline-block">
                        {prompt.category}
                      </span>
                      <span className="font-mono text-[9px] sm:text-xs font-bold bg-white border-2 border-black px-2 py-0.5 sm:px-2.5 sm:py-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                        🔥 {Math.round(prompt.trendingScore || 0)} PTS
                      </span>
                    </div>
                    
                    <h3 className="font-display font-black text-lg sm:text-2xl md:text-3xl uppercase leading-[1.1] mb-1 sm:mb-2 break-words line-clamp-2">
                      {prompt.title}
                    </h3>
                    
                    <div className="text-[10px] sm:text-sm font-mono text-black font-bold mb-1 sm:mb-4">
                      by @{prompt.profiles?.username}
                    </div>

                    <p className="font-sans text-[10px] sm:text-sm font-bold text-neutral-900 line-clamp-2 sm:line-clamp-5 leading-relaxed border-l-4 border-black pl-2 sm:pl-4 bg-white/30 py-1 sm:py-2">
                      {prompt.prompt_text}
                    </p>
                  </div>

                  {/* Actions & Stats */}
                  <div className="flex flex-wrap items-center justify-between mt-3 pt-3 sm:mt-8 sm:pt-6 border-t-4 border-black gap-2 sm:gap-4 relative z-20">
